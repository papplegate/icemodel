...
class TestAll:
    ...
    def test_instances_are_typed(self, chinook: sqlite3.Connection) -> None:
        _results = tuple(Artist.query().limit(1))
...
class TestWhere:
    def test_equality(self, chinook: sqlite3.Connection) -> None:
        results = tuple(Artist.query().where(Artist.Fields.NAME, "AC/DC"))
...
class TestWhereIn:
    def test_where_in(self, chinook: sqlite3.Connection) -> None:
        results = tuple(Artist.query().where_in(Artist.Fields.ARTISTID, [1, 2, 3]))
        assert {a.ArtistId for a in results} == {1, 2, 3}

    def test_empty_where_in_returns_nothing(self, chinook: sqlite3.Connection) -> None:
        results = tuple(Artist.query().where_in(Artist.Fields.ARTISTID, []))
        assert results == ()
...
class TestFirst:
    def test_first_returns_one(self, chinook: sqlite3.Connection) -> None:
        _results = tuple(Track.query().order_by(Track.Fields.TRACKID).limit(1))
...
class TestToSql:
    def test_to_sql_simple(self, chinook: sqlite3.Connection) -> None:
        sql, params = Artist.query().to_sql()
        assert sql == "SELECT * FROM Artist"
        assert params == []

    def test_to_sql_with_where(self, chinook: sqlite3.Connection) -> None:
        sql, params = Artist.query().where(Album.Fields.ARTISTID, 1).to_sql()
        assert sql == "SELECT * FROM Artist WHERE ArtistId = ?"
        assert params == [1]
    ...
    def test_to_sql_with_order_by(self, chinook: sqlite3.Connection) -> None:
        sql, params = Artist.query().order_by(Artist.Fields.NAME).to_sql()
        assert sql == "SELECT * FROM Artist ORDER BY Name ASC"
        assert params == []
    ...
    def test_to_sql_with_limit(self, chinook: sqlite3.Connection) -> None:
        sql, params = Artist.query().limit(10).to_sql()
        assert sql == "SELECT * FROM Artist LIMIT 10"
        assert params == []

    def test_to_sql_with_limit_and_offset(self, chinook: sqlite3.Connection) -> None:
        sql, params = Artist.query().limit(10).offset(20).to_sql()
        assert sql == "SELECT * FROM Artist LIMIT 10 OFFSET 20"
        assert params == []
    ...
    def test_to_sql_empty_where_in(self, chinook: sqlite3.Connection) -> None:
        sql, params = Artist.query().where_in(Artist.Fields.ARTISTID, []).to_sql()
        assert sql == "SELECT * FROM Artist WHERE 1 = ?"
        assert params == [0]
